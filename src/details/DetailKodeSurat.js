...
const DetailKodeSurat = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3001/kodeSurat/${id}`, {
        ...
        if (response.data.status) {
          setKodeSurat(response.data.message.KodeSurat);
  ...
  const handleUpdateClickk = async () => {
    try {
      ...
      // update role
      const response = await axios.patch(`http://localhost:3001/kodeSurat/${id}`,
        {
          KodeSurat: kodeSurat,
      ...
      if (response) {
        Swal.fire({
          icon: 'success',
          title: 'Berhasil mengupdate kode surat!',
          confirmButtonColor: '#198754'
        });
        navigate('/dataKodeSurat');

      } else {
        Swal.fire({
          icon: 'error',
          title: 'Gagal mengupdate kode surat!',
          confirmButtonColor: '#198754'
        });
      }
    } catch (error) {
            ...
            <div className='col-lg-12' style={{ backgroundColor: "white", borderRadius: "5px", marginRight: "15px" }}>
              <nav aria-label="breadcrumb" style={{ marginTop: "10px", marginBottom: "10px" }}>
                <ol className="breadcrumb">
                  <li className="breadcrumb-item active" aria-current="page"><b style={{ color: "black" }}>Detail Kode Surat</b></li>
                </ol>
              </nav>
            </div>
