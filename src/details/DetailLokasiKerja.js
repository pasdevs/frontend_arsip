...
const DetailLokasiKerja = () => {
  ...
  const handleChangeLokasiKerja = (event) => {
    setLokasiKerja(event.target.value);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3001/lokasiKerja/${id}`, {
        ...
        if (response.data.status) {
          setLokasiKerja(response.data.message.LokasiKerja);
  ...
  const handleUpdateClickk = async () => {
    try {
      if (!lokasiKerja) {
        setLokasiKerjaError("Lokasi Kerja harus diisi.");
      }
      if (!keterangan) {
      ...
      // update role
      const response = await axios.patch(`http://localhost:3001/lokasiKerja/${id}`,
        {
      ...
      if (response) {
        Swal.fire({
          icon: 'success',
          title: 'Berhasil mengupdate lokasi kerja!',
          confirmButtonColor: '#198754'
        });
        navigate('/dataLokasiKerja');

      } else {
        Swal.fire({
          icon: 'error',
          title: 'Gagal mengupdate lokasi kerja!',
          confirmButtonColor: '#198754'
        });
      }
    } catch (error) {
      console.log("erornya apa?", error)
      Swal.fire({
        icon: 'error',
          ...
          <div className='row'>
            <div className='col-lg-12' style={{ backgroundColor: "white", borderRadius: "5px", marginRight: "15px" }}>
              <nav aria-label="breadcrumb" style={{ marginTop: "10px", marginBottom: "10px" }}>
                <ol className="breadcrumb">
                  <li className="breadcrumb-item active" aria-current="page"><b style={{ color: "black" }}>Detail Lokasi Kerja</b></li>
                </ol>
              </nav>
            </div>
            ...
            <div className='row' style={{ backgroundColor: "#F5F5F7", borderRadius: "5px", marginLeft: "0px", marginRight: "0px", marginBottom: "10px", padding: "10px" }}>
              ...
              <div className='col-lg-12'>
                <div className="mb-3">
                  <label htmlFor="lokasiKerja" className="form-label" style={{ fontSize: "small" }}>Lokasi Kerja</label>
                  <div className="input-group">
                    <input
                      ...
                      value={lokasiKerja}
                      onChange={handleChangeLokasiKerja}
                    />
