...
const TablePengguna = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3001/pengguna', {
  ...
  const deleteData = async (id) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3001/pengguna/${id}`, {
        headers: { 'X-CSRF-Token': resultCsrf, 'Content-Type': 'application/json' },
        withCredentials: true
      });
            ...
            <div className='col-lg-12' style={{ backgroundColor: "white", borderRadius: "5px", marginRight: "15px" }}>
              <nav aria-label="breadcrumb" style={{ marginTop: "10px", marginBottom: "10px" }}>
                <ol className="breadcrumb">
                  <li className="breadcrumb-item active" aria-current="page"><b style={{color: "black"}}>Data Pengguna</b></li>
                </ol>
              </nav>
            </div>
          </div>
            ...
            <div className='col-lg-12' style={{ marginTop: "20px" }}>
              <div className='row'>
                ...
                <div className='col' style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                  <Link to="/formPengguna"><button type="button" className="btn btn-success btn-sm">Tambah Pengguna</button></Link>
                </div>
              </div>
            </div>
                ...
                <div className="card-header">
                  <div className='row'>
                    <div className='col' style={{ display: "flex", alignItems: "center" }}>
                      <b style={{ fontSize: "14px" }}>Data Pengguna</b>
                    </div>
                    <div className='col' style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                    ...
                    <tbody>
                      {currentItems.map((item, index) => (
                        <tr key={item.RoleID}>
                          ...
                          <td>{item.Username}</td>
                          <td>{item.PasswordDef}</td>
                          {/* <td>{item.PasswordDef !== "" || item.PasswordDef !== null ? "••••••••" : ""}</td> */}
                          <td>{item.Role}</td>
                          <td>{item.Status}</td>
