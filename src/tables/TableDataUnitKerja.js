...
const TableDataUnitKerja = () => {
  ...
  const deleteData = async (id) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3001/unitKerja/${id}`, {
        headers: { 'X-CSRF-Token': resultCsrf, 'Content-Type': 'application/json' },
        withCredentials: true,
        params: {
          userToken: userToken
            ...
            <div className='col-lg-12' style={{ backgroundColor: "white", borderRadius: "5px", marginRight: "15px" }}>
              <nav aria-label="breadcrumb" style={{ marginTop: "10px", marginBottom: "10px" }}>
                <ol className="breadcrumb">
                  <li className="breadcrumb-item active" aria-current="page"><b style={{ color: "black" }}>Data Unit Kerja</b></li>
                </ol>
              </nav>
            </div>
          </div>
            ...
            <div className='col-lg-12' style={{ marginTop: "20px" }}>
              <div className='row'>
                ...
                <div className='col' style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                  <Link to="/formDataUnitKerja"><button type="button" className="btn btn-success btn-sm">Tambah Unit Kerja</button></Link>
                </div>
              </div>
            </div>
                ...
                <div className="card-header">
                  <div className='row'>
                    <div className='col' style={{ display: "flex", alignItems: "center" }}>
                      <b style={{ fontSize: "14px" }}>Data Unit Kerja</b>
                    </div>
                    <div className='col' style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                  ...
                  <table className="table table-bordered table-hover text-nowrap table-sm" style={{ marginBottom: "0px", fontSize: "14px" }}>
                    ...
                    <tbody>
                      {filteredData.map((item, index) => (
                        <tr key={item.UnitKerjaID}>
                          <td>{(currentPage) * itemsPerPage + index + 1}</td>
                          <td>{item.UnitKerja}</td>
                          <td>{item.Keterangan}</td>
                          <td>
                            <div>
                              <FontAwesomeIcon icon={faLayerGroup} onClick={() => handleDetail(item.UnitKerjaID)} data-toggle="tooltip" title="Detail" data-placement="top" style={{ marginRight: "10px", cursor: 'pointer', color: "black" }} />
                              <FontAwesomeIcon icon={faTrashCan} onClick={() => handleDelete(item.UnitKerjaID)} style={{ cursor: 'pointer', color: "red" }} data-toggle="tooltip" title="Hapus" data-placement="top" />
                            </div>
                          </td>
                        </tr>
                      ))}
